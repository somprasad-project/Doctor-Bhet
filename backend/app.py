...
def load_data():
    """Load all required datasets and model"""
    try:
        data = {
        ...
        logger.info("All data and model loaded successfully")
        return data
...
def helper(dis: str) -> Tuple[str, List, List, List, List]:
    """Get all related information for a disease"""
    try:
        desc_df = data['description'][data['description']['Disease'] == dis]
        desc = " ".join(desc_df['Description'].values) if not desc_df.empty else "No description available"
        pre_df = data['precautions'][data['precautions']['Disease'] == dis]
        ...
        logger.error(f"Error in helper function for disease {dis}: {str(e)}")
...
def get_predicted_value(patient_symptoms: List[str]) -> str:
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            prediction = data['model'].predict(input_vector)[0]
...
def index():
    ...
    return jsonify({'message': 'Welcome to the Medical Recommendation API'}), 200
...
def get_symptoms():
    ...
    try:
        ...
        logger.error(f"Symptoms list error: {str(e)}")
        return jsonify({'error': 'Failed to load symptoms'}), 500
